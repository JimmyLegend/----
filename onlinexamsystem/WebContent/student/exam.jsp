...
<%@page import="com.sdut.examsystem.service.teacher.QuestionService"%>
...
return true;
	...
	<%
		...
		List qWenDaList = (List) session.getAttribute("qWenDaList");//问答
		int k = 1;
	%>
	<div id="main_content">
	...
	<div >
		<form id="form1" name="form1" method="post"
			action="<%=path %>/studentPaperServlet?method=save&testId=<%=test.get("id") %>">
			<div id="fd">
 ...
 */
 ...
 idt=window.setTimeout("update();",1000);
			...
			<!-- 选择题 -->
			<p>
				<b>一、单项选择题（100分）（每小题<%=sin_score %>分，错选、未选均不得分。）</b>
			</p>
			<%
				
				for (int i = 0; i < sin_ques.size(); i++) {
					Map map = (Map)sin_ques.get(i);
			%>
			<p></p>
			<p class="text"><%=k%>.<%=map.get("queTitle") %></p>
			<p>
				A.<%=map.get("choiceA") %></p>
			<p>
				B.<%=map.get("choiceB") %></p>
			...
			<p class="text">
				<label> <input type="radio"
					name="choice_<%=map.get("id") %>" value="A" id="choice_A" />
					A.
				</label> <label> <input type="radio"
					name="choice_<%=map.get("id") %>" value="B" id="choice_B" />
					B.
				</label> <label> <input type="radio"
					name="choice_<%=map.get("id") %>" value="C" id="choice_C" />
					C.
				</label> <label> <input type="radio"
					name="choice_<%=map.get("id") %>" value="D" id="choice_D" />
					D.
				</label>
			</p>
			<%
			...
			<!-- 判断题 -->
			<p>
				<b>一、判断题（100分）（每小题<%=sin_score %>分，错选、未选均不得分。）</b>
			</p>
			<%
				
				for (int i = 0; i < qPanDuanList.size(); i++) {
					Map map = (Map)qPanDuanList.get(i);
			%>
			<p></p>
			<p class="text"><%=k%>.<%=map.get("queTitle") %></p>
			<!-- <p>
