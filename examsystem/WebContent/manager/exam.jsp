		...
		<div>
			<form id="form1" name="form1" method="post" action="<%=path %>/studentTest?&testId=${test.id}">
				...
				<c:forEach var="que" items="${quesList}" varStatus="status">
					...
					<p>D.${que.choiceD }</p>
					<p>
					A.<input type="radio" name = "quesxuanze_${que.id}" value = "A"/>
					B.<input type="radio" name = "quesxuanze_${que.id}" value = "B"/>
					C.<input type="radio" name = "quesxuanze_${que.id}" value = "C"/>
					D.<input type="radio" name = "quesxuanze_${que.id}" value = "D"/>
					</p>
				</c:forEach>
				...
				<c:forEach var="que" items="${qPanDuanList}" varStatus="status">
					...
					-->
					<p>
					是.<input type="radio" name = "quespanduan_${que.id}" value = "YES"/>
					否.<input type="radio" name = "quespanduan_${que.id}" value = "NO"/>
				...
				<p>&nbsp;</p>
				<center><textarea id="paperTitle" name="paperTitle" cols="100" rows="5" style="width:750px;height:260px;visibility:hidden;float:left;">${testQuestions.paperTitle}</textarea></center>
