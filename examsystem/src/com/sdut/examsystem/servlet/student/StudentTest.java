...
//		System.out.println(wrongAns.toString());
		...
		if (quesList.size()>wrongQueNum)
			{
			p.setScore(1.0*Integer.parseInt(testMap.get("scores").toString())/quesList.size()*(quesList.size()-wrongQueNum));
		...
		if (qPanDuanList.size()>wrongPanDuanQueNum)
			{
			p.setPanDuanScore(1.0*Integer.parseInt(testMap.get("panduanscores").toString())/qPanDuanList.size()*(qPanDuanList.size()-wrongPanDuanQueNum));
		...
		if (qTianKongList.size()>wrongTianKongQueNum)
			{
			p.setTianKongScore(1.0*Integer.parseInt(testMap.get("tiankongscores").toString())/qTianKongList.size()*(qTianKongList.size()-wrongTianKongQueNum));
